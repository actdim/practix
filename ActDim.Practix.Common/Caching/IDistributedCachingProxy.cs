...
{
    /// <summary>
    /// Memoizing proxy over an <see cref="IDistributedCache"/>: wraps a value-producing function
    /// into a delegate that serves cached values and only invokes the underlying function on a miss.
    /// </summary>
    public interface IDistributedCachingProxy
    {
        /// <summary>
        ...
        /// </summary>
        Func<string, T> Get<T>(Func<string, T> func, DistributedCacheEntryOptions options);
    }
}
