...
namespace ActDim.Practix.Extensions // ActDim.Practix.Linq
{
	...
	public static class MathExtensions
	{
        ...
        {
            return value == (Int64)value; // System.Convert.ToInt64(value) or Math.Floor(value) or Math.Ceiling(value) or Math.Round(value)
        }

        public static bool HasDecimalPart(float value)
        {
            return value == (Int32)value; // System.Convert.ToInt32(value)
        }
		...
		/// <param name="start">The number to start from.</param>
		/// <param name="bound">The number to stop before.</param>
		public static IEnumerable<int> To(this int start, int bound) { var step = start <= bound ? 1 : -1; for (int i = start; i < bound; i += step) yield return i; }
		/// <summary>Generates a sequence of 32-bit signed integers within a specified range.</summary>
		/// <returns>A sequence of numbers from start to (but not including) bound where each number is a given step greater than the previous number.</returns>
		/// <param name="start">The number to start from.</param>
		/// <param name="bound">The number to stop before.</param>
        ...
        {
			from = from.Clamp(fromMin, fromMax);
        ...
        public static float Remap(this byte from, byte fromMin, byte fromMax, byte toMin, byte toMax)
        {
            from = from.Clamp(fromMin, fromMax);
