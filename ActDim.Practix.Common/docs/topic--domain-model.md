...
### 2. Ambient Execution Context Lifecycle
```
[AmbientContext.Default]
    |
    +---> using (AmbientContext.WithServices(provider))
    |        |-- Pushes new ImmutableDictionary to AsyncLocal
    |        |-- Executes child async branch
    |        +-- On Scope Dispose(): Reverts AsyncLocal to previous dictionary
```
