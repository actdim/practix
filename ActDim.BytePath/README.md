...
High-performance keyed blob and binary payload storage engine with metadata, expiration, and distributed read/write locking.
...
```bash
# Core engine
dotnet add package ActDim.BytePath

# Storage & Registry backends
...
Register `BlobManager` fluently with backends:
...
{
    // Fluent builder attaching FileSystem store & SQLite registry:
    services.AddBlobManager()
        .WithFileSystemDataStore(baseDirectory: "./data/blobs")
        .WithSQLiteRegistry(databaseName: "registry.db");

    // Or with custom implementations (e.g. S3 + PostgreSQL):
    // services.AddBlobManager()
    //     .WithDataStore<CustomS3DataStore>()
    //     .WithRegistry<PostgreSqlBlobRegistry>();
}
```
