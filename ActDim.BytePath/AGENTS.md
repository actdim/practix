...
Blob and binary payload storage engine split into two layers that know nothing about each other:
...
IBlobRegistry.cs           # public: metadata + lock contract
SQLiteBlobRegistry.cs      # public: lock engine + persistence
FileSystemBlobDataStore.cs # public: FS implementation

Tests/BytePath.Tests/BlobManagerTests.cs   # 69 xUnit v3 tests
```
...
- Namespace and assembly: `ActDim.BytePath`; target `net10.0`; `Nullable` disabled
- Public: `IBlobManager`, `IBlobDataStore`, `IBlobRegistry`, `BlobStoreOptions`, `BlobRecord`, `BlobResult`,
  `BlobErrorCode`, `LockType`, `FileSystemBlobDataStore`, `SQLiteBlobRegistry`, `BlobManagerServiceCollectionExtensions` (`AddBlobManager`)
- Internal: `BlobManager`, `BlobRecordTransport`, `BlobManagerBuilder`
- `InternalsVisibleTo("ActDim.BytePath.Tests")`: tests construct internals directly
