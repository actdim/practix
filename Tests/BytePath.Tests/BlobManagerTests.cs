...
namespace ActDim.BytePath.Tests
{
    public class BlobManagerTests
    {
        ...
        [Fact]
        public void AddBlobManager_FluentBuilder_ConfiguresDataStoreAndRegistry()
        {
            ...
            try
            {
                ...
                services.AddBlobManager()
                    .WithFileSystemDataStore(tempDir)
                    .WithSQLiteRegistry(customDbName, tempDir);
        ...
        [Fact]
        public void AddBlobManager_DelegateConfiguration_ConfiguresServices()
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "blob_test_" + Guid.NewGuid().ToString("N"));
            const string customDbName = "custom_registry.sqlite";
            ...
            try
            {
                ...
                {
                    builder.WithFileSystemDataStore(tempDir);
                    builder.WithSQLiteRegistry(customDbName, tempDir);
                });
        ...
        [Fact]
        public void AddBlobManager_CustomInstances_PreservesCustomDataStoreAndRegistry()
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "blob_test_" + Guid.NewGuid().ToString("N"));
            var customDataStore = new FileSystemBlobDataStore(tempDir);
            var customRegistry = new SQLiteBlobRegistry(Path.Combine(tempDir, "custom.db"));

            try
            {
                var services = new ServiceCollection();
                services.AddBlobManager()
                    .WithDataStore(customDataStore)
                    .WithRegistry(customRegistry);

                var provider = services.BuildServiceProvider();
                var resolvedDataStore = provider.GetService<IBlobDataStore>();
                var resolvedRegistry = provider.GetService<IBlobRegistry>();
                var resolvedManager = provider.GetService<IBlobManager>();

                Assert.NotNull(resolvedManager);
                Assert.Same(customDataStore, resolvedDataStore);
                Assert.Same(customRegistry, resolvedRegistry);
            }
            finally
            {
                if (Directory.Exists(tempDir))
