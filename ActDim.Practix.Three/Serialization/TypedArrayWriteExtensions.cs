...
namespace THREE.Serialization
{
    ...
    internal static class TypedArrayWriteExtensions
    {
        public static void WriteTo(this ITypedArray values, JsonWriter writer)
        {
            ...
            switch (values)
            {
                case Int8Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Uint8Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Uint8ClampedArray a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Int16Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Uint16Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Int32Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Uint32Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Float16Array a: foreach (var v in a.Data) { writer.WriteValue((float)v); } break;
                case Float32Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
                case Float64Array a: foreach (var v in a.Data) { writer.WriteValue(v); } break;
            }
