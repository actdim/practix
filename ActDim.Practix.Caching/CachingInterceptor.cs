...
namespace ActDim.Practix.Caching
{
    internal class CachingInterceptor<T> : IInterceptor // where T : class
    {
        ...
        // private static readonly ConcurrentDictionary<string, Lazy<SemaphoreSlim>> LockMap = new ConcurrentDictionary<string, Lazy<SemaphoreSlim>>();
        private static readonly IDictionary<string, SemaphoreSlim> LockMap = new Dictionary<string, SemaphoreSlim>();
                    ...
                    else
                    {
                        ...
                        if (isCached)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    var entryValue = _jsonSerializer.DeserializeObjectFromBson(stream, entryValueType); // valueContainer

                                    // var propertyInfo = TypeAccessor.GetProperty(() => default(IContainer).Value);
                                    var propertyInfo = TypeAccessor.GetPropertyInfo((IContainer c) => c.Value);
                ...
                else
                {
                    ...
                    var action = new Action<object>(result =>
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var entryValueType = typeof(ValueContainer<>).MakeGenericType(returnType); // valueContainerType
                                var entryValueCtor = TypeAccessor.GetConstructorEx(entryValueType, returnType); // valueContainerCtor
                                var entryValue = entryValueCtor(result);
                                using (var stream = new MemoryStream())
                                {
                                    _jsonSerializer.SerializeObjectToBson(entryValue, stream);
