...
- **Central Package Management (CPM) & Central Versioning**: Repository fully standardized on NuGet CPM via root `Directory.Packages.props` and central versioning in `Directory.Build.props` (**Version 1.0.12**).
...
- **Total Test Suite**: 640 tests passing in `ActDim.Practix.sln` plus 39 in `ActDim.Three.sln` with zero failures (679 / 679 total tests passing).
